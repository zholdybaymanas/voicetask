...
export default function Dashboard() {
  ...
  function triggerVoice() {
    window.dispatchEvent(new CustomEvent('voiceInputTrigger'))
  }
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h2 className="text-xl font-semibold text-text truncate">{getGreeting(displayName)}</h2>
        ...
        >
          ...
          <span className="hidden sm:inline">Голосовая задача</span>
        </button>
      </div>
      ...
      {overdueTasks.length > 0 && (
        <section>
          <h3 className="text-sm font-semibold text-text mb-2 flex items-center gap-2">
            ...
            Просроченные
            <span className="text-muted font-normal">({overdueTasks.length})</span>
          </h3>
          <div className="bg-card rounded-xl border border-border shadow-card divide-y divide-border">
            {overdueTasks.map(t => (
              <TaskRow
